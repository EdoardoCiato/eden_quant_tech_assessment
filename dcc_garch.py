...
def _dcc_loglik(params, std_resids):
    ...
    # unconditional covariance matrix of the standardised residuals
    Qbar = e.T @ e / T  # (2, 2)

    # Initialise Q_0 = Qbar because we have no prior information. 
    Q = Qbar.copy()

    llh = 0.0
    # recursion for all the datapoints. 
    for t in range(1, T):
        # reshape necessary to perform matrix multiplication. 
        e_lag = e[t - 1, :].reshape(-1, 1)   
        # standard equation dynamic conditional correlation
        Q = (1 - alpha - beta) * Qbar  + alpha * (e_lag @ e_lag.T) + beta  * Q
...
def main():
    ...
    possible_stocks = [
        {'ticker': 'Orion Chips',  'path': 'data/orion_eod.csv'},
        {'ticker': 'Cleanergy',    'path': 'data/cleanergy_eod.csv'},
        {'ticker': 'NovaTerra AI', 'path': 'data/novaterra_eod.csv'},
    ]
